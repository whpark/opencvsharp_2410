...
namespace OpenCvSharp.BlobCv4
{
    ...
    public class CvBlobs : Dictionary<int, CvBlob>, ICloneable
    {
        ...
        public CvBlobs(Mat img)
        {
            ...
            if (img.Depth() != MatType.CV_8U || img.Channels() != 1)
        ...
        public CvScalar BlobMeanColor(CvBlob targetBlob, Mat originalImage)
        {
            ...
            unsafe
            {
                ...
                {
                    byte* imgData = (byte*)originalImage.Ptr(r);
        ...
        public void FilterLabels(Mat imgOut)
        {
            ...
            unsafe
            {
                ...
                for (int r = 0; r < h; r++)
                {
                    byte* imgData = (byte*)imgOut.Ptr(roi.Top+r);
                    for (int c = 0; c < w; c++)
                    {
                        ...
                        if (label != 0)
                        {
                            if (ContainsKey(label))
                                imgData[roi.Left + c] = 0xff;
                            else
                                imgData[roi.Left + c] = 0x00;
                        }
                        else
                        {
                            imgData[roi.Left + c] = 0x00;
